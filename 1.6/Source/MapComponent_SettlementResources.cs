...
namespace DanielRenner.SettledIn
{
    ...
    public  class MapComponent_SettlementResources : MapComponent
    {
        ...
        public bool SettlementCenterAlreadyBuiltBefore = false;
        ...
        public List<Pawn> WantsMorePersonalSpace = new List<Pawn>();
        public List<Pawn> WantsFewerTasks = new List<Pawn>();
        public List<Pawn> WantsMoreRecreation = new List<Pawn>();

        // stores the beds assigned to pawns
        public Dictionary<Pawn, Building> PawnBeds = new Dictionary<Pawn, Building>();

        // stores the unliked pawns with nearby beds to other pawns
        public Dictionary<Pawn, List<Pawn>> UnlikedNeighborsSleepingNearby = new Dictionary<Pawn, List<Pawn>>();
        ...
        public override void MapComponentTick()
        {
            ...
            if (currTicks % 62500 == 0) // daily calculate a chance for immigrants
            {
                ...
                if (SettlementLevelUtility.IsBenefitActiveAt(SettlementLevel, SettlementLevelUtility.Benefit_lvl0_Immigrants) && Rand.Chance(chanceForImmigrants)) // only active starting with settlement level 1
                {
                    ...
                    /* fully manual version usign own quest types; removed due to quest generation error with V1.6
                    ...
                    */
                    ...
                    // Generate threat points like storyteller does
                    float points = StorytellerUtility.DefaultThreatPointsNow(map);

                    // Prepare the quest slate
                    Slate slate = new Slate();
                    slate.Set("map", map);
                    slate.Set("points", points);
                    //slate.Set("asker", null); // No specific quest giver
                    slate.Set("arrivalMode", "WalkIn");
                    slate.Set("joinerFaction", Faction.OfPlayer);
            ...
            if ((currTicks + 767) % 62500 == 0) // daily calculate the happiness due to construction work, but unalign it with the default modulo for spreading the calculation time each frame
            {
                if (SettlementLevelUtility.IsBenefitActiveAt(SettlementLevel, SettlementLevelUtility.Benefit_lvl1_ManagedSettlements))
                {
                    ...
                    PawnBeds.Clear();
                    UnlikedNeighborsSleepingNearby.Clear(); // todo: think about memory improvements to not always throw away that many lists
                    WantsWorkbench.Clear();
                    LongCommuters.Clear();
                    WantsMorePersonalSpace.Clear();
                    ...
                    // prepare some lists regarding which buildings are in which rooms and how these contribute to the pawn expectations on personal belonging space
                    var buildingsInRooms = new Dictionary<Room, List<Building>>();
                    var pawnsAssignedToRooms = new Dictionary<Room, List<Pawn>>();
                    foreach (var building in allBuildingsAssignedToPawns)
                    {
                        var room = building.GetRoom();
                        if (!buildingsInRooms.ContainsKey(room))
                            buildingsInRooms[room] = new List<Building>();
                        buildingsInRooms[room].Add(building);
                        if (!pawnsAssignedToRooms.ContainsKey(room))
                            pawnsAssignedToRooms[room] = new List<Pawn>();
                        var assignedPawns = building.GetComp<CompAssignableToPawn>().AssignedPawns;
                        foreach (var assignedPawn in assignedPawns)
                        {
                            if (!pawnsAssignedToRooms[room].Contains(assignedPawn))
                                pawnsAssignedToRooms[room].Add(assignedPawn);
                        }
                    }
                    // calculate the room scores per pawn based on the assigned buildings and the score of the rooms they are in
                    var roomScoreAttributedToPawn = new Dictionary<Pawn, float>();
                    foreach (var room in pawnsAssignedToRooms.Keys)
                    {
                        // find the statistic for that room
                        /*
                        cachedStatistics.roomRoles.Find(roomRoleStatistic => { 
                            return roomRoleStatistic.rooms
                        });*/
                        float totalScoreHere = room.CellCount;
                        var perPawnScore = totalScoreHere / pawnsAssignedToRooms[room].Count;
                        foreach (Pawn pawnInThisRoom in pawnsAssignedToRooms[room])
                        {
                            if (!roomScoreAttributedToPawn.ContainsKey(pawnInThisRoom))
                                roomScoreAttributedToPawn[pawnInThisRoom] = 0f;
                            roomScoreAttributedToPawn[pawnInThisRoom] += perPawnScore;
                        }
                    }
...
#endif
                                if (!UnlikedNeighborsSleepingNearby.ContainsKey(pawn))
                                    UnlikedNeighborsSleepingNearby[pawn] = new List<Pawn>();
                                // find the specific pawns and beds that cause this
                                var closeByUnlikedBeds = unlikedBeds.Where(unlikedBed => { return myBed.Position.DistanceTo(unlikedBed.Position) < unlikedPawnBedDistance; });
                                foreach (var unlikedBed in closeByUnlikedBeds)
                                {
                                    foreach (var assignedPotentiallyUnlikedPawn in unlikedBed.GetAssignedPawns())
                                    {
                                        if (unlikedPawns.Contains(assignedPotentiallyUnlikedPawn))
                                        {
                                            UnlikedNeighborsSleepingNearby[pawn].Add(assignedPotentiallyUnlikedPawn);
                                        }
                                    }
                                }
                                if (!HasUnlikedNeighbours.Contains(pawn))
                            ...
                            */
                            ...
                            if (SettlementLevelUtility.IsBenefitActiveAt(SettlementLevel, SettlementLevelUtility.Benefit_lvl1_PersonalWorkbench))
                            {
                                ...
                                foreach (var skillType in validSkillTypes)
                                {
                                    ...
                                    highestSkill = Math.Max(highestSkill, currLevel);
                                ...
                                // too small personal space: 
                                // get the expectation of that pawn. The highest skill * 3
                                var roomScoresForThisPawn = roomScoreAttributedToPawn[pawn];
                                var pawnExpectation = highestSkill * 3;
                                var deltaExpectation = roomScoresForThisPawn / pawnExpectation;
                                if (deltaExpectation >= 1.5f)
                                {
                                    // awesome! positive buff
#if DEBUG
                                    var prevMemory = pawn.needs.mood.thoughts.memories.GetFirstMemoryOfDef(DefOfs_SettledIn.EnoughPrivateSpace);
                                    if (prevMemory == null)
                                    {
                                        Log.Debug(pawn.Name + ": Assigning new buff for EnoughPrivateSpace");
                                    }
#endif
                                    pawn.needs.mood.thoughts.memories.TryGainMemory((Thought_Memory)ThoughtMaker.MakeThought(DefOfs_SettledIn.EnoughPrivateSpace));

                                }
                                else if (deltaExpectation < 1.0f)
                                {
                                    // bad expectation
#if DEBUG
                                    var prevMemory = pawn.needs.mood.thoughts.memories.GetFirstMemoryOfDef(DefOfs_SettledIn.NotEnoughPrivateSpace);
                                    if (prevMemory == null)
                                    {
                                        Log.Debug(pawn.Name + ": Assigning new buff for NotEnoughPrivateSpace");
                                    }
#endif
                                    if (!WantsMorePersonalSpace.Contains(pawn))
                                    {
                                        WantsMorePersonalSpace.Add(pawn);
                                    }
                                    pawn.needs.mood.thoughts.memories.TryGainMemory((Thought_Memory)ThoughtMaker.MakeThought(DefOfs_SettledIn.NotEnoughPrivateSpace));

                                }
                                // todo: add a neutral buff?
        ...
        // temporary lists required by Scribe for Dictionaries
        private List<Pawn> tmpPawnKeys;
        private List<Building> tmpBedValues;

        private List<Pawn> tmpHaterKeys;
        private List<List<Pawn>> tmpHaterValues;
        ...
        public override void ExposeData()
        {
            ...
            try
            {
                ...
                Scribe_Values.Look(ref SettlementCenterAlreadyBuiltBefore, "SettlementCenterAlreadyBuiltBefore", false);
                ...
                Scribe_Collections.Look(ref PawnBeds, "PawnBeds", LookMode.Reference, LookMode.Reference, ref tmpPawnKeys, ref tmpBedValues);
                Scribe_Collections.Look(ref UnlikedNeighborsSleepingNearby, "UnlikedNeighborsSleepingNearby", LookMode.Reference, LookMode.Reference, ref tmpHaterKeys, ref tmpHaterValues);

                if (Scribe.mode == LoadSaveMode.PostLoadInit)
                {
                    if (PawnBeds == null) 
                        PawnBeds = new Dictionary<Pawn, Building>();
                    if (UnlikedNeighborsSleepingNearby == null) 
                        UnlikedNeighborsSleepingNearby = new Dictionary<Pawn, List<Pawn>>();
                }
